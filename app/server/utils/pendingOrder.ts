...
/**
 * The payload as it comes back out of the DB. `comp` is null when the stored JSON
 * could not be parsed (see mapRow) — readers must guard before using it.
 */
export interface StoredPendingPayload {
  input: OrderInput
  comp: OrderComputation | null
}
...
function mapRow(r: RowDataPacket): PendingRow {
  let payload: StoredPendingPayload
  try {
    payload = JSON.parse(String(r.payload))
  } catch {
    payload = {
      input: { items: [], shippingMethod: 'abholung', paymentMethod: 'vorkasse' },
      comp: null,
    }
  }
  return {
...
export async function getPendingByToken(db: Pool, token: string): Promise<PendingRow | null> {
  const [rows] = await db.execute<RowDataPacket[]>(
    'SELECT * FROM koop_pending_order WHERE token = ? LIMIT 1',
    [token],
  )
...
export async function getPendingById(db: Pool, id: number): Promise<PendingRow | null> {
  const [rows] = await db.execute<RowDataPacket[]>(
    'SELECT * FROM koop_pending_order WHERE id = ? LIMIT 1',
    [id],
  )
  return rows[0] ? mapRow(rows[0]) : null
}
...
export async function getPendingByOrderId(db: Pool, ordersId: number): Promise<PendingRow | null> {
  const [rows] = await db.execute<RowDataPacket[]>(
    'SELECT * FROM koop_pending_order WHERE orders_id = ? LIMIT 1',
    [ordersId],
  )
  return rows[0] ? mapRow(rows[0]) : null
}
...
export async function cancelPending(
  ...
  if (pending.status === 'materialized') {
    throw createError({
      statusCode: 409,
      statusMessage: 'Bereits bestätigte Bestellung kann nicht storniert werden',
    })
  }
  await dbUpdate(
    db,
