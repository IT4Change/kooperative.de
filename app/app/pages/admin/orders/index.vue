          ...
          >
            ...
            <td class="px-4 py-2.5">
              <span
                ...
                :class="
                  o.kind === 'pending' ? 'bg-amber-100 text-amber-800' : statusClass(o.statusId)
                "
              >
...
<script setup lang="ts">
  ...
  const { data, error, pending } = await useFetch<OrderList>('/admin/api/orders', {
    query: apiQuery,
  })
  ...
  function goto(o: OrderRow) {
    void router.push(o.kind === 'pending' ? `/admin/pending/${o.id}` : `/admin/orders/${o.id}`)
  }
</script>
