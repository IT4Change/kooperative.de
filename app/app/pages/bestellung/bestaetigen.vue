...
<script setup lang="ts">
  ...
  const { data, error, refresh } = await useFetch<PendingView>(
    () => `/api/orders/pending/${token.value}`,
    {
  ...
  const isConfirmed = computed(
    () =>
      justConfirmed.value ||
      data.value?.status === 'materialized' ||
  ...
  async function confirm() {
    ...
    try {
      ...
      confirmError.value =
        err.data?.statusMessage || err.statusMessage || 'Bestätigung fehlgeschlagen.'
    } finally {
